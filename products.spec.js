...
test.describe('🛍️ Products Page Tests', () => {
  ...
  test.describe('📋 Product Listing', () => {
    ...
    test('TC_PROD_002 - Page title should be Products', async ({ page }) => {
      await expect(page.locator('.title')).toHaveText('Products');
    });

    test('TC_PROD_003 - All products have a name', async ({ page }) => {
      const names = await page.locator('.inventory_item_name').allTextContents();
      names.forEach(name => expect(name.length).toBeGreaterThan(0));
    });

    test('TC_PROD_004 - All products have a price', async ({ page }) => {
      const prices = await page.locator('.inventory_item_price').allTextContents();
      prices.forEach(price => expect(price).toMatch(/\$\d+\.\d{2}/));
    });

    test('TC_PROD_005 - All products have an image', async ({ page }) => {
    ...
    test('TC_PROD_006 - All products have Add to Cart button', async ({ page }) => {
  ...
  test.describe('🔢 Sorting', () => {
    ...
    test('TC_SORT_002 - Sort Z-A works correctly', async ({ page }) => {
    ...
    test('TC_SORT_003 - Sort Price Low to High works', async () => {
    ...
    test('TC_SORT_004 - Sort Price High to Low works', async () => {
  ...
  test.describe('🛒 Add to Cart', () => {
    ...
    test('TC_CART_002 - Cart count updates with multiple products', async () => {
      await productsPage.addProductToCart(PRODUCTS.backpack);
    ...
    test('TC_CART_003 - Button changes to Remove after adding', async ({ page }) => {
      await productsPage.addProductToCart(PRODUCTS.backpack);
